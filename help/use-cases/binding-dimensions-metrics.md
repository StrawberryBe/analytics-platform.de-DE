...
1. Ein Besucher gelangt zu Ihrer Site und sucht „Boxhandschuhe“.
...
1. Er findet ein Paar Handschuhe, die ihm gefallen, und fügt sie zu seinem Warenkorb hinzu.
...
1. Er findet einen Schläger, der ihm gefällt, und fügt ihn seinem Warenkorb hinzu.
...
1. Der Besucher sucht ein drittes Mal, diese Mal „Schuhe“.
...
1. Er findet ein Paar Schuhe, die ihm gefallen, und fügt sie zu seinem Warenkorb hinzu.
...
Wenn Sie nun ein herkömmliches Zuordnungsmodell mit Suchbegriff verwenden, weisen alle drei Produkte den Umsatz nur einem einzigen Suchbegriff zu. Wenn Sie beispielsweise die erste Zuordnung mit der Suchbegriffdimension verwendet haben:
...
Bei einer Bindungsdimension nimmt Adobe das Dimensionselement zur Kenntnis, an das es gebunden ist. Wenn derselbe Bindungswert in einem nachfolgenden Ereignis angezeigt wird, überträgt er das Dimensionselement, damit Sie ihm die gewünschte Metrik zuordnen können. In diesem Beispiel können wir die Bindungsdimension für search_term auf den Produktnamen setzen. Wenn wir diese Dimension im Datenansichts-Manager festlegen, müssen wir auch eine Bindungsmetrik festlegen, da sich die Bindungsdimension in einem Objekt-Array befindet. Eine Bindungsmetrik dient als Trigger für eine Bindungsdimension, sodass sie sich nur an Ereignisse bindet, bei denen die Bindungsmetrik vorhanden ist. In dieser Beispielimplementierung enthält die Suchergebnisseite immer eine Suchbegriffdimension und eine Suchmetrik. Wir können Suchbegriffe an den Produktnamen binden, wann immer die Suchmetrik vorhanden ist.
...
* Wenn die Suchmetrik vorhanden ist, binden Sie den Suchbegriff an alle Produktnamen. Er verhält sich so, als ob er auf derselben Ebene wie der Produktname für dieses Ereignis wäre. In diesem Beispiel wird er als product.search_term behandelt.
