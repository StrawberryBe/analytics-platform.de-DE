...
Adobe Customer Journey Analytics ist Analytics-Produkt der nächsten Generation. Dieser Artikel enthält Antworten auf häufig gestellte Fragen zum Customer Journey Analytics. Weitere Informationen finden Sie unter [Customer Journey Analytics-Funktionsunterstützung](/help/getting-started/aa-vs-cja/cja-aa.md).
...
Customer Journey Analytics beinhaltet [Datenvorbereitung](https://experienceleague.adobe.com/docs/experience-platform/data-prep/api/overview.html?lang=de) Funktionen, mit denen Sie Ihre Daten umwandeln können, bevor Sie sie in den Adobe Experience Platform Data Lake einfügen. Wenn Sie einen ETL-Prozess benötigen, nachdem die Daten aufgenommen wurden, stehen Ihnen über den [Abfrage-Service von Adobe Experience Platform](https://experienceleague.adobe.com/docs/platform-learn/tutorials/queries/understanding-query-service.html?lang=de#queries) einige begrenzte Optionen zur Verfügung. Hierfür könnten jedoch zusätzliche Gebühren anfallen.
...
Ja. [!UICONTROL Customer Journey Analytics] verfügt über eine [Zuordnungsfunktion](../stitching/overview.md), die innerhalb eines Datensatzes für authentifizierte und nicht authentifizierte Ereignissen verwendet werden kann. Diese Zuordnung ermöglicht die Auflösung verschiedener Datensätze zu einer einzigen zugeordneten ID für geräteübergreifende Analysen auf der Personenebene.
...
**Beispielszenario**: Sie verbinden zwei Datensätze in einer Customer Journey Analytics-Verbindung mithilfe von `CRMid` als Personen-ID. Einer ist ein Web-Ereignis-Datensatz mit `CRMid` in allen Datensätzen. Der andere Datensatz ist ein CRM-Profildatensatz. 40 % des CRM-Datensatzes verfügen über `CRMid`, das im Web-Ereignis-Datensatz vorhanden ist. Die anderen 60 % sind nicht im Webereignis-Datensatz vorhanden. Werden diese Datensätze in der Berichterstellung in Analysis Workspace angezeigt?<p> **Antwort**: Profilzeilen, für die keine Ereignisse verknüpft sind, werden in Customer Journey Analytics gespeichert. Sie können sie jedoch erst dann in Analysis Workspace sehen, wenn ein mit dieser ID verknüpftes Ereignis angezeigt wird.
...
Solange die Personen-ID zwischen Datensätzen übereinstimmt, [!UICONTROL Customer Journey Analytics] kann Filter, Attribution, Fluss, Fallout usw. über Datensätze hinweg verbinden.
...
<ul><li>Für vergangene Daten/Zeitstempel: Ereignisdaten bis zu zehn Jahre alt.</li><li>Für zukünftige Datumsangaben/Zeitstempel: Ereignisdaten (prädiktiv) bis zu einem Monat in der Zukunft.</li></ul>
...
>[!NOTE]
>In Customer Journey Analytics gibt es keine feste Datengröße. Daher kann Adobe sich nicht auf eine standardmäßige Aufnahmezeit festlegen. Adobe arbeitet aktiv daran, diese Latenzen durch neue Updates und Erfassungsoptimierung zu reduzieren.

<ul><li>Live-Daten oder -Ereignisse: Werden innerhalb von 90 Minuten verarbeitet und aufgenommen, sobald Daten in Adobe Experience Platform verfügbar sind. (Batch-Größe &gt; 50 Millionen Zeilen: länger als 90 Minuten.)</li><li>Kleine Backups: innerhalb von sieben Tagen<li>Große Aufstockungen: Innerhalb von 30 Tagen</li></ul>
...
| Aktion | Auswirkung |
| --- | --- |
| Löschen einer Sandbox in [!UICONTROL Adobe Experience Platform] | Das Löschen einer Sandbox stoppt den Datenfluss zu allen [!UICONTROL Customer Journey Analytics] Verbindungen zu Datensätzen in dieser Sandbox herzustellen. Derzeit werden [!UICONTROL Verbindungen] in Customer Journey Analytics, die zur gelöschten Sandbox bestehen, nicht automatisch gelöscht. |
...
| Datensatz in der [!UICONTROL Adobe Experience Platform] Datensee | Durch das Löschen eines Datensatzes im Adobe Experience Platform Data Lake wird der Datenfluss von diesem Datensatz zu allen Customer Journey Analytics-Verbindungen, die diesen Datensatz umfassen, angehalten. Alle Daten aus diesem Datensatz werden automatisch aus den zugehörigen Customer Journey Analytics-Verbindungen gelöscht. |
...
| Löschen von einem Batch, **während er** in [!UICONTROL Customer Journey Analytics] aufgenommen wird | Wenn nur ein Batch im Datensatz vorhanden ist, werden keine Daten oder nur teilweise Daten aus diesem Batch in [!UICONTROL Customer Journey Analytics]. Die Aufnahme wird zurückgesetzt. Wenn beispielsweise fünf Batches im Datensatz vorhanden sind und drei davon bereits erfasst wurden, als der Datensatz gelöscht wurde, werden Daten aus diesen drei Batches in [!UICONTROL Customer Journey Analytics]. |
| Löschen einer Verbindung in [!UICONTROL Customer Journey Analytics] | Eine Fehlermeldung weist auf Folgendes hin:<ul><li>für die gelöschte Verbindung erstellte Datenansichten nicht mehr funktionieren.</li><li> Ebenso funktionieren alle Workspace-Projekte, die von Datenansichten in der gelöschten Verbindung abhängig sind, nicht mehr.</li></ul> |
...
Wenn Sie Adobe Analytics-Daten über die [Adobe Analytics-Quell-Connector](https://experienceleague.adobe.com/docs/experience-platform/sources/connectors/adobe-applications/analytics.html?lang=de)berücksichtigen Sie diese Auswirkungen beim Zusammenführen von zwei oder mehr Adobe Analytics Report Suites.
...
Noch nicht, aber Adobe arbeitet aktiv daran, diese Funktion bereitzustellen.

+++

+++**Was ist mit meiner alten [!UICONTROL eVar]-Einstellung passiert?**

[!UICONTROL eVars], [!UICONTROL Eigenschaften] und [!UICONTROL Ereignisse] im Sinne von Adobe Analytics existieren in [!UICONTROL Customer Journey Analytics] nicht mehr. Es gibt unbegrenzte Schemaelemente (Dimensionen, Metriken, Listenfelder). Alle Attributionseinstellungen, die Sie während des Datenerfassungsprozesses angewendet haben, werden jetzt zur Abfragezeit angewendet.
...
[!UICONTROL Customer Journey Analytics] verwendet keine eVars, Eigenschaften oder Ereignisse mehr, sondern nutzt stattdessen ein beliebiges Adobe Experience Platform-Schema. Dies bedeutet, dass keine der vorhandenen Segmente oder berechneten Metriken mit [!UICONTROL Customer Journey Analytics].
...
Das hängt von Ihrem Anwendungsfall ab, also wenden Sie sich an Ihr Adobe Account-Team. Ihr aktueller Anwendungsfall ist möglicherweise bereits gut für Customer Journey Analytics geeignet!
...
Nehmen wir beispielsweise an, Ihr Vertrag berechtigt Sie zu einer Million Datenzeilen. Angenommen, Sie laden am 1. Tag der Verwendung von Customer Journey Analytics zwei Millionen Datenzeilen hoch. Am Tag 2 löschen Sie 1 Million Zeilen und behalten Ihre Nutzung für den Rest Ihrer Lizenzvereinbarung bei (d. h. maximal eine Million Datenzeilen). Abhängig von Ihren Vertragsbedingungen können Ihnen für Tag 1 anteilige zusätzliche Nutzungsgebühren entstehen, da Sie die Lizenzberechtigung für „Datenzeilen“ überschritten haben.

## 11. Diskrepanzen bei Daten erkennen {#discrepancies}

Manchmal kann es vorkommen, dass sich die Gesamtanzahl der von Ihrer Verbindung erfassten Ereignisse von der Anzahl der Zeilen im Datensatz in [!UICONTROL Adobe Experience Platform]. In diesem Beispiel enthält der Datensatz „B2B Impression“ 7650 Zeilen, der Datensatz enthält jedoch 3830 Zeilen in [!UICONTROL Adobe Experience Platform]. Es kann mehrere Gründe für Diskrepanzen geben. Die folgenden Schritte können für die Diagnose hilfreich sein:

1. Schlüsseln Sie diese Dimension nach **[!UICONTROL Platform-Datensatz-ID]** und Sie sehen zwei Datensätze mit derselben Größe, aber unterschiedlicher Größe **[!UICONTROL Platform-Datensatz-IDs]**. Jeder Datensatz enthält 3.825 Aufzeichnungen. Das bedeutet [!UICONTROL Customer Journey Analytics] fünf Datensätze aufgrund fehlender Personen-IDs oder fehlender Zeitstempel ignoriert wurden:

   ![Aufschlüsselung](assets/data-size2.png)

1. Wenn Sie außerdem einchecken [!UICONTROL Adobe Experience Platform], gibt es keinen Datensatz mit der ID &quot;5f21c12b732044194bffc1d0&quot;, daher hat jemand diesen bestimmten Datensatz aus [!UICONTROL Adobe Experience Platform] bei der Erstellung der ersten Verbindung. Später wurde er erneut zu Customer Journey Analytics hinzugefügt. Es wurde jedoch eine andere [!UICONTROL Platform-Datensatz-ID] von [!UICONTROL Adobe Experience Platform] generiert.
...
Die Adobe Experience Cloud verwendet die regionale Datenerfassung (Regional Data Collection, RDC), damit Interaktionen zwischen Ihren Besuchern und Adobe- und Nicht-Adobe-Lösungen so nahe wie möglich an Ihren Besuchern stattfinden. Sobald Daten regional in einem Datenerfassungscenter (DCC, auch als Edge-Site bezeichnet, Teil des Platform Edge-Netzwerks) erfasst wurden, werden sie über eine sichere Verbindung zu den relevanten Lösungen weitergeleitet, die auf der Konfiguration Ihres Datastreams und/oder der Ereignisweiterleitung basiert.

![Datenfluss mithilfe von Edge Networks](https://experienceleague.adobe.com/docs/experience-platform/assets/collection.png?lang=en)
...
* **Redundanz**: Wenn die Kommunikation zwischen dem DCC und Ihrem DPC unterbrochen wird, speichert die Adobe RDC-Infrastruktur Daten lokal und leitet sie bei der Wiederherstellung der Kommunikation an den DPC weiter.
...
Für die Customer Journey Analytics sind Datensätze aus Adobe Experience Platform erforderlich. Daher muss der Adobe Experience Platform-Dienst für Ihre Konfiguration der Datenspeicherung/Ereignisweiterleitung die Daten vom regionalen Rechenzentrum zum Rechenzentrum weiterleiten, in dem sich Ihre Adobe Experience Platform-Instanz befindet. Customer Journey Analytics und die zugehörigen unterstützenden Dienste und Infrastrukturen werden in derselben Adobe Experience Platform-Instanz bereitgestellt.


Siehe [Datenerfassung - Übersicht](https://experienceleague.adobe.com/docs/experience-platform/collection/home.html?lang=en) für weitere Informationen über den Prozess der Datenerfassung außerhalb des Experience Edge-Netzwerks und seiner regionalen Rechenzentren.
